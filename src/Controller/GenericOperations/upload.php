<?php
include_once MY_DOC_ROOT . '/model/api/assettype.php';
...
class GenericUpload{
  ...
  private function prepare_asset($mime, $id, &$filepath, &$filename, &$tip, $oldfile = null){
        ...
        $this->response['message'] = 'The file is invalid, please use a valid file (jpg, png, bmp, gif, mp3, ogg, wav)';
				$this->response['code'] = 415;
			}
		}else{
			$this->response['type'] = 'error';
      $this->response['title'] = 'Invalid format';
      $this->response['message'] = 'The file format is invalid, please use a valid format (jpg, png, bmp, gif, mp3, ogg, wav)';
			$this->response['code'] = 415;
		}
